...
function TeacherLiveClass() {
    ...
    useEffect(() => {
        ...
        peer.on('call', (call) => {
            startSelfVideo().then((mediaStream) => {
                ...
                console.log("student peerid:",call.peer)
    ...
    const handleEndClass = async () => {
        try {
            // Notify the backend that the class has ended
            if (socket) {
            ...
            // Stop the local media stream (camera and microphone)
            if (stream) {
                stream.getTracks().forEach((track) => {
                    track.stop(); // Stop the track
                    console.log(`Track stopped: ${track.kind}`); // Debugging log
                });
            ...
            if (currentUserVideoRef.current) {
                currentUserVideoRef.current.srcObject = null; // Clear video source
            }
            if (remoteVideoRef.current) {
                remoteVideoRef.current.srcObject = null; // Clear video source
            }

            // Navigate to the teacher's classes view
            navigate("/teacher-view-class-course");
        } catch (error) {
            ...
            <div className="live-class-content">
                ...
                <div className="video-section row">
                    ...
                    <div className='col student-video-section'>
                        <video ref={remoteVideoRef} className="student-video me-2" />
                    </div>
                </div>
