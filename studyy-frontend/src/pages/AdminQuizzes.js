  ...
  const getQuizzez = async () => {
    try {
      ...
      if (data.status === "ok") {
        ...
        console.log("message from admin get quizzes:", message)
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const indexOfLastQuiz = currentPage * quizPerPage;
        const indexOfFirstQuiz = indexOfLastQuiz - quizPerPage;
        const updatedCurrentQuizzes = updatedQuizzes.slice(indexOfFirstQuiz, indexOfLastQuiz);
        setCurrentQuizzes(updatedCurrentQuizzes);
      } else {
              ...
              <tbody>
                {currentQuizzes.map((quiz, index) => (
                  <tr key={quiz._id}>
                    ...
                    <td>
                      <button className='btn table-button mx-1' onClick={() => handleView(quiz)}>View</button>
                      <button className='btn table-button mx-1'
